...
>   - `CaretLineAffinityTests` 6개, 876 → 882. 반증 3종(기하 무시 / End가 안 세움 / 클램프 제거) 각각
>     해당 테스트만 빨개진다.
>   > **미포함: 클릭 affinity.** 감긴 줄 마지막 글자의 뒤쪽 절반을 클릭하면 종전대로 다음 줄에 선다.
>   > 그 경로는 `HitTestLogicalIndex`/`HitTestIndex`가 **오프셋만** 돌려주므로 affinity를 빼내려면 별도
>   > 변경이 필요하다(포트도 그 자리에서 한다). 회귀는 없다 — 이번은 키보드 절반이다.
