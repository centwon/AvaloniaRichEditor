...
- **착수 기준**: 수백 페이지 초대형 문서에서 프레임 드랍이 실측될 때. → **충족(2026-06-11)**: N6-6 실측에서 100장 문서 스크롤 29fps(50장까지는 50fps+). 관리 Render는 4ms로 양호 — 병목은 씬 크기(래스터화)이므로 Draw 컬링이 유효. 단 yOffset 누적·캐럿 좌표 계산은 유지하고 **Draw 호출만 생략**하는 보수적 구현으로(히트테스트 3곳 무관, `_lastCaretPoint`/`BringIntoView` 보존).
