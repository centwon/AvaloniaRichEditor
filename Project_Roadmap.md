...
>   테스트 11건 추가(366→377).
> - **남은 것(설계 판단 필요)**: ↑/↓로 표 **셀 진입** 불가(현재는 블록 캐럿에 멈췄다가 표를 건너뜀 — 의도된 모델인지 결정 필요), IME 조합 중 셀 행 높이가 안 자람(measure 경로가 preedit 미인지), ~~동기 `ParseHtml` 원격 이미지~~ → **수정 완료**(동기 경로는 네트워크 I/O 없음, 원격은 `ParseHtmlAsync`)

기능 충실도 **A**, 코드 품질 **B+**, 견고성/검증 **B**, 프로덕션 준비도 **B−(베타)**.
혼자 만든 from-scratch Avalonia 리치 에디터로는 상위권 — 기능은 상용 근접, 1.0은 *새 기능이 아니라 검증 깊이*로 결정.

**완성된 기능 (전부 동작·테스트 그린, 377 unit + 9 render):**
