...
- [ ] **`v1.0.0` 태그 push** → CI `pack` 잡이 Trusted Publishing으로 nuget.org 게시 (사람이 실행)
- [ ] 첫 PR에서 **macOS CI 실제 그린** 확인(P1 이후 미검증)
...
>   - 다만 **동작 차이 1건은 기록해 둔다**(수정 안 함, 판단 필요): 읽기 전용에서 이미지를 우클릭하면
>     포트는 **이미지 메뉴(이미지 자체를 Copy)** 를 주고 여기는 일반 복사+모두 선택을 준다 — 여기서는
>     그 Copy가 이미지가 아니라 (대개 비어 있는) 텍스트 선택을 복사한다. 포트 쪽이 낫다.
