...
>   **최상위 문단도 조합 중 스크롤 extent가 안 자람**(실측 65 vs 65 — `BlockExtent`가 preedit 미인지. 셀과 달리 렌더는 preedit
>   높이로 진행하므로 겹침은 없고 extent만 한 줄 짧음. `BlockExtent`의 `out paraLayout`이 히트테스트 오프셋 매핑에 쓰여서
>   같은 방식으로 고치면 조합 중 클릭 위치가 틀어짐 — 설계 판단 필요해 보류),
...
**완성된 기능 (전부 동작·테스트 그린, 393 unit + 9 render):**
