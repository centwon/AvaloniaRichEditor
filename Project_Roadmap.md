...
- [~] **게시 진행 중(2026-06-13, 사용자 결정 — `0.2.0-alpha`)**: 전수 검증(Release 빌드·163테스트·왕복·pack·AOT) 통과 후 게시 준비 완료 — 버전 0.2.0-alpha 범프, `PackageIcon`(icon.png 생성·동봉 확인), CHANGELOG 0.2.0-alpha 절 확정, **PublicAPI Unshipped 368건 → Shipped 승격**, ci.yml에 Trusted Publishing 게시 스텝(OIDC, `NuGet/login`, 시크릿 없음). **잔여: 사용자가 nuget.org에 신뢰 게시 정책 등록(저장소 centwon/AvaloniaRichEditor + 워크플로 ci.yml) → `v0.2.0-alpha` 태그 푸시 → GitHub Release.** (이전 보류 결정 2026-06-10은 해제 — alpha 프리릴리스가 API 변동을 커버하므로.)
