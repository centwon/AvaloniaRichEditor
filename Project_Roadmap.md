...
**완성된 기능 (전부 동작·테스트 그린, 784 unit + 25 render):**
...
- **줄 간격의 HTML/RTF 왕복** — 두 포매터 모두 줄 간격을 다루지 않아 내보내면 사라진다. HTML `line-height: 1.6`은 HWP 기준과
  1:1, RTF `\slmult`는 Word "배수"라 ÷1.2 근사가 필요하다(라운드24).
- **WinUI 포트에 라운드24 반영** — 줄 간격 HWP 기준·기본 160%·↓ 다음 블록 걸음. 포트의 줄 높이 API 동작부터 확인.
...
- **게시 후 nuget.org 패키지 페이지를 눈으로 확인한다.** README는 `.nupkg` 안에 들어가고 nuget.org는
  raw HTML을 렌더하지 않는다 — `<p align="center"><img>`가 태그 그대로 본문에 찍혔고, GitHub에서는
  멀쩡해 보여서 **1.1.0부터 5주 동안 아무도 몰랐다**(1.2.1로 수정). 테스트가 구조적으로 못 보는 자리다.
