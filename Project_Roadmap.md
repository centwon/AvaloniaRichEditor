...
**P1에서 새로 드러난 후속 항목**
- `RichEditorLocalization.Language`를 **UI 스레드 밖에서** 바꾸면 붙어 있는 툴바의 `Build()`가 그 스레드에서 돌아
  스레드 친화성 예외로 죽는다(P1 테스트가 이 경로로 깨졌다). 호스트가 백그라운드에서 언어를 바꿀 수 있으니
  `OnLanguageChanged`에서 UI 스레드로 넘기는 게 맞다 — 1.0 전에 판단.
