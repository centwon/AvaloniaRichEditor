...
**완성된 기능 (전부 동작·테스트 그린, 446 unit + 9 render):**
...
**우선순위는 라운드3의 실측 결과가 정한다** — 결함 24건 중 **11건이 GUI 육안검증에서만** 드러났고
전부 포인터·포커스·키 조합 계열이었다. 446개 테스트가 구조적으로 못 보는 영역이다.

- [ ] **P1 · 상호작용 테스트 인프라** ⭐ 최우선
      헤드리스 `new Window{ Content = ed }.Show()` + `MouseDown/MouseMove/MouseUp/KeyPress/KeyTextInput`.
      **실현 가능성 실측 완료(2026-07-29)**: 입력 테스트 4건을 전체 스위트에 섞어 449/450 통과, 기존 445건 전원 통과.
      **`Close()`는 여전히 금지**(플랫폼 종료 → 후속 테스트 붕괴). `Show()` 없이는 입력이 컨트롤에 도달하지 않음.
      **macOS CI는 미검증** — 첫 PR에서 확인할 것.
      대상: 툴바 포커스, 리사이즈 드래그 배선(현재 헬퍼 직접 호출로만 검증), 셀 드래그 선택, 우클릭 메뉴 라우팅.
- [ ] **P2 · 렌더 픽셀 테스트 깊이** — `Tests.Render`(real Skia) 9건 → 복잡 표·페이지 분할·인라인 표까지 확대
- [ ] **P3 · 상호운용 격차**
      RTF 표/인라인 표 내보내기(현재 HTML만 보존, HWP는 RTF 선호) ·
      HTML 인라인 표 재가져오기(현재 블록 표로 들어옴; in-app 메타로 보존 가능)
- [ ] **P4 · 성능 실측** — `--bench`를 인라인 표 포함으로 재측정 +
      **IME 조합 중 표 체인 캐시 무효화 비용**(라운드3에서 추가된 경로, 조합 글자마다 실행)
...
**릴리스 시 정리할 것**
- 위 "라운드2/3" 블록이 70줄 넘게 부풀었다 → `docs/roadmap-archive.md`로 이관하고 이 절은 요약만 남긴다
- `CLAUDE.md`의 커밋 트레일러가 `Claude Opus 4.8`인데 실제 커밋은 `Opus 5` — 어느 쪽으로 통일할지 미결
