...
> - **🔴 새로 발견 (미수정)**: **줄 오른쪽 여백을 클릭하면 캐럿이 문단 길이+1에 놓임.** `HitTestIndex`가 Avalonia의
>   `TextPosition + IsTrailing`을 그대로 반환하는데 줄 끝 너머에서 length+1이 나온다. CJK·길이 무관하게 재현
>   (`'ab'`→3, `'hello world'`→12). 확인된 결과 2가지: **①Backspace가 아무것도 안 지움**, **②그 위치에서 타이핑하면
>   기존 run 서식이 끊긴 새 run이 생김**(굵은 줄 뒤에 입력 → 보통 글씨). 일상 동작이라 영향이 큼. 수정은 `HitTestIndex`에서
>   문단 길이로 클램프하는 방향(단, 모든 히트테스트 소비처에 영향).
> - **남은 것(설계 판단 필요)**: ↑/↓로 표 **셀 진입** 불가(현재는 블록 캐럿에 멈췄다가 표를 건너뜀 — 의도된 모델인지 결정 필요),
>   ~~동기 `ParseHtml` 원격 이미지~~ → **수정 완료**(동기 경로는 네트워크 I/O 없음, 원격은 `ParseHtmlAsync`)

기능 충실도 **A**, 코드 품질 **B+**, 견고성/검증 **B**, 프로덕션 준비도 **B−(베타)**.
혼자 만든 from-scratch Avalonia 리치 에디터로는 상위권 — 기능은 상용 근접, 1.0은 *새 기능이 아니라 검증 깊이*로 결정.

**완성된 기능 (전부 동작·테스트 그린, 395 unit + 9 render):**
