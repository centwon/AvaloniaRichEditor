...
- [ ] **Phase 2 — 편집 뷰 페이지 모드**: y-리매핑 배선(렌더·히트테스트 3곳·캐럿·선택·BringIntoView), 페이지 크롬(흰 종이+회색 데스크+그림자+페이지 사이 갭), `StyledProperty`로 on/off. N6-5 Draw 컬링과 좌표 정합 확인.
- [ ] **Phase 3 — 페이지 렌더 + 미리보기**: 페이지 단위 300DPI `RenderTargetBitmap` 렌더 API + 인쇄 미리보기 창(데모).
- [ ] **Phase 4 — 출력**: Windows 프린터 인쇄(데모, `System.Drawing.Printing`) + 래스터 PDF 라이터(라이브러리, 무의존).
- 후속(소형, 페이지네이터 확보 후): 머리말/꼬리말/쪽번호, 표 행 경계 페이지 분할.
