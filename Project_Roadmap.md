...
- **IME 한글 기본 입력(추후)**: 한국어 Windows에서 에디터 포커스 시 IME를 한글 모드로 자동 시작. Windows는 IMM32 `ImmSetConversionStatus(IME_CMODE_NATIVE|FULLSHAPE)` P/Invoke로 시도(창 HWND `TopLevel.TryGetPlatformHandle`). 코드는 ~30줄로 작으나 **Win10/11 TSF IME에서 IMM32가 안 먹을 위험** → 대안(`SendInput(VK_HANGUL)`/TSF)으로 길어질 수 있음. 직접 검증 불가라 실기 테스트 필요. 언어 감지는 쉬움. 개념은 OS 보편이나 자동전환 구현은 플랫폼별.
- **남은 보류 항목**:
