...
- [ ] **남은 갭**: `RichEditor` private 경로(`DeleteLocalText`/`TryInsertTextCore`/Undo 코얼레싱)와 붙여넣기 폴백 체인(내부리치→HTML→평문, async `IClipboard` 의존)은 미커버 — 클립보드 페이크 필요해 비용 중간.
...
- [~] 테스트 커버리지 확대 — **오프셋 모델(인라인 이미지=1글자)·멀티문단 삭제/스타일·부분 서식 분할 10건 완료(2026-06-10, 37→47)**. 남은 것: `RichEditor` private 편집 경로(`DeleteLocalText`/`TryInsertTextCore`)·붙여넣기 폴백(async 클립보드 페이크 필요).
