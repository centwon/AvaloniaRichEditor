...
/// <summary>Reads an image's pixel dimensions directly from its encoded header, without decoding it.
/// From the WinUI port (2026-09-16): the renderer needs the source size to decode a picture at the size it
/// is drawn, and the size presets need it for "original size" — both used to decode the whole bitmap (and
/// keep it on the model) just to read <c>.Size</c>.</summary>
